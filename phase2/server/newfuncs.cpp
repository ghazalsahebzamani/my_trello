...
void user_check(Trello& _trello,int identifier,string& user){
	for(int i=0;i<_trello.get_members().size();i++)
	{
		if(_trello.get_members()[i]->get_user()==user)
		{
			send(identifer,"UserTaken()");
		}
	}
	send(identifer,"ok");
return;
...
void filter_interface(Trello& _trello,istream& in,ostream& out){
	...
	_trello.get_current_board()->filter_lists(users,in,out);
	}catch(MyException& exp){
		out<<exp.what()<<endl;
	}	
}
void show_my_cards_interface(Trello& _trello,istream& in,ostream& out){
	try{
		...
		_trello.get_current_board()->show_member_lists(user,in,out);
	}catch(MyException& exp){
		out<<exp.what()<<endl;
	}
}
...
void server_event_check(char c,Trello& _trello,ostream& out,istream& in){
	switch(c)
	{
		case '1': string user=server::receive();
					user_check()
					Trello& _trello,ostream& out,istream& in,int identifier,string& user
