...
void Board::filter_lists(vector<Member*> users,string des_check,string cm_check,ostream& out){
	cerr<<"ok2?dc:"<<des_check<<"cc:"<<cm_check<<endl;
	int err_num=0;
	for(int i=0;i<lists.size();i++)
		try{
			lists[i]->filter_cards(users,des_check,cm_check,out);
		}catch(NoCardsFound& exp){
			err_num++;
		}
		if(err_num==lists.size())
...
void Board::show_member_lists(Member* user,string des_check,string cm_check,ostream& out){
	int err_num=0;
	for(int i=0;i<lists.size();i++)
		{
			try{				
			lists[i]->show_member_cards(user,des_check,cm_check,out);
